...
const AdminDashboardComponent = ({
    ...
    return (
        <div className="flex min-h-screen bg-gray-100 bg-cover bg-center" style={{ backgroundImage: `url(${bgImage})` }}>
            {/* Sidebar */}
            <div className="w-1/4 bg-gray-900 text-white p-6 flex flex-col space-y-4 min-h-screen">
                <h2 className="text-2xl font-bold">Admin Dashboard</h2>
                <button onClick={handleCreateRoom} className={`py-2 px-4 rounded transition ${viewCreate ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-700"}`}>
                    Create a Room
                </button>
                <button onClick={handleViewEditRooms} className={`py-2 px-4 rounded transition ${viewRooms ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-700"}`}>
                    View and Edit Rooms
                </button>
                <button onClick={handleViewBookings} className={`py-2 px-4 rounded transition ${viewBookings ? "bg-blue-500 hover:bg-blue-600" : "bg-gray-700"}`}>
                    View Bookings
                </button>
            </div>
            ...
            <div className="flex-1 p-6">
                ...
                {/* Welcome Message */}
                {!viewCreate && !viewRooms && !viewBookings&&(
                    <div className="flex flex-col items-center justify-center mt-20">
                        <h1 className="text-5xl font-bold text-gray-800 mb-4">Welcome, Admin!</h1>
                        <p className="text-xl text-gray-600">Manage your meeting rooms efficiently.</p>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {meetingRooms.map((room) => (
                                <div key={room.id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
                                    ...
                                    <p className="text-gray-700 mb-4">Capacity: {room.capacity}</p>
                                    <div className="flex space-x-4">
                                        <button onClick={() => handleUpdate(room)} className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-300">
                                            Update
                                        </button>
                                        <button onClick={() => handleDelete(room.id)} className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 transition duration-300">
                                            Delete
                                        </button>
                                    </div>
                                </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {bookings.map((booking) => (
                                <div key={booking.id} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
                                    <h2 className="text-2xl font-bold mb-2 text-gray-800">{booking.meetingRoomName}</h2>
<p className="text-gray-700 mb-2">Booked By: {booking.userName}</p>
<p className="text-gray-700 mb-4">Start Time: {booking.startTime}</p>
<p className="text-gray-700 mb-4">End Time: {booking.endTime}</p>
<p
key={booking.status}
  className={`mb-4 ${
